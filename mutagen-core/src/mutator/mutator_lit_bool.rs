    ...
    pub fn transform(e: Expr, transform_info: &SharedTransformInfo) -> ExprTransformerOutput {
        match e {
            Expr::Lit(ExprLit {
                ...
                let mutator_id =
                    transform_info.add_mutation(Mutation::new_spanned("lit_bool".to_owned(), span));
...
mod tests {
    ...
    pub fn false_inactive() {
        let result =
            MutatorLitBool::run(1, false, MutagenRuntimeConfig::with_mutation_id(0));
        assert_eq!(result, false)
    }
    #[test]
    pub fn true_inactive() {
        let result =
            MutatorLitBool::run(1, true, MutagenRuntimeConfig::with_mutation_id(0));
        assert_eq!(result, true)
    }
    #[test]
    pub fn false_active() {
        let result =
            MutatorLitBool::run(1, false, MutagenRuntimeConfig::with_mutation_id(1));
        assert_eq!(result, true)
    }
    #[test]
    pub fn true_active() {
        let result =
            MutatorLitBool::run(1, true, MutagenRuntimeConfig::with_mutation_id(1));
        assert_eq!(result, false)
    }
