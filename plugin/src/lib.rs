        ...
        expr.and_then(|expr| match expr {
            ...
            Expr {
                ...
                BinOpKind::And => {
                    ...
                    let left = left.map(|e| self.fold_expr(P(e)));
                    let right = right.map(|e| self.fold_expr(P(e)));
            ...
            Expr {
                ...
                let cond = cond.map(|e| self.fold_expr(P(e)));
                let then = fold::noop_fold_block(then, self);
                let opt_else = opt_else.map(|p_else| p_else.map(|e| self.fold_expr(P(e))));
